...
async def terminal_control():
    ...
    print("\n" + "="*50)
    print("🤖 HỆ THỐNG ĐIỀU KHIỂN BOT TERMINAL ĐÃ SẴN SÀNG!")
    print("Gõ 'help' hoặc 'h' để xem danh sách lệnh.")
    print("="*50 + "\n")

    while not bot.is_closed():
        try:
            status_prompt = f"[{selected_guild.name}]" if selected_guild else "[Chưa chọn Server]"
            cmd_raw = await asyncio.to_thread(input, f"CMD {status_prompt} > ")
            ...
            # 1. PING
            if command in ["1", "ping"]:
                latency = round(bot.latency * 1000, 2)
                print(f"📡 [PING] Độ trễ (Latency): {latency} ms\n")

            # 2. SERVERS
            elif command in ["2", "servers"]:
                guilds = list(bot.guilds)
                if not guilds:
                    print("❌ Bot chưa tham gia server nào!\n")
                    continue
                    
                print("\n--- 🌐 DANH SÁCH SERVER BOT ĐANG THAM GIA ---")
                for idx, guild in enumerate(guilds, start=1):
                    is_sel = " 👈 [Đang chọn]" if selected_guild and selected_guild.id == guild.id else ""
                    print(f" [{idx}] {guild.name} | ID: {guild.id} | Thành viên: {guild.member_count}{is_sel}")
                print("---------------------------------------------\n")

            # 3. SELECT
            elif command in ["3", "select"]:
                guilds = list(bot.guilds)
                if not guilds:
                    print("❌ Bot chưa tham gia server nào!\n")
                    continue
                ...
                if not target_stt:
                    ...
                    target_stt = await asyncio.to_thread(input, "👉 Nhập STT Server cần chọn (gõ 'exit' để hủy): ")
                    target_stt = target_stt.strip()

                if target_stt.lower() == "exit":
                    print("🔙 Đã hủy thao tác chọn Server.\n")
                    continue

                if not target_stt.isdigit():
                    print("❌ Vui lòng nhập số thứ tự hợp lệ!\n")
                    continue

                idx = int(target_stt) - 1
                if 0 <= idx < len(guilds):
                    selected_guild = guilds[idx]
                    print(f"✅ Đã chọn Server: [{selected_guild.name}] (ID: {selected_guild.id})\n")
                else:
                    print("❌ STT Server không tồn tại!\n")

            # 4. CHANNELS
            elif command in ["4", "channels"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server! Hãy dùng lệnh 2 (servers) rồi 3 (select <STT>) trước.\n")
                    continue

                print(f"\n--- 📺 DANH SÁCH KÊNH CHAT TRONG [{selected_guild.name}] ---")
                text_channels = [ch for ch in selected_guild.channels if isinstance(ch, discord.TextChannel)]
                for idx, ch in enumerate(text_channels, start=1):
                    print(f" [{idx}] #{ch.name} (ID: {ch.id})")
                print("---------------------------------------------------\n")

            # 5. SEND
            elif command in ["5", "send"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server! Hãy dùng lệnh 3 (select) để chọn server trước.\n")
                    continue

                send_data = args
                if not send_data:
                    print("\n--- KÊNH CHAT TRONG SERVER ---")
                    text_channels = [ch for ch in selected_guild.channels if isinstance(ch, discord.TextChannel)]
                    for idx, ch in enumerate(text_channels, start=1):
                        print(f" [{idx}] #{ch.name}")
                    print("------------------------------")
                    
                    send_data = await asyncio.to_thread(input, "👉 Nhập dạng: <STT/ID kênh> | <Nội dung> (gõ 'exit' để hủy): ")
                ...
                if "|" not in send_data:
                    print("❌ Cú pháp sai! Phải có dấu `|` ngăn cách.\nVí dụ: 8 | Hello mọi người\n")
                    continue

                target_channel_str, message_text = [x.strip() for x in send_data.split("|", 1)]
                text_channels = [ch for ch in selected_guild.channels if isinstance(ch, discord.TextChannel)]
                
                target_obj = None
                if target_channel_str.isdigit():
                    val = int(target_channel_str)
                    if 1 <= val <= len(text_channels):
                        target_obj = text_channels[val - 1]
                    else:
                        target_obj = selected_guild.get_channel(val)

                if target_obj and isinstance(target_obj, discord.TextChannel):
                    try:
                        await target_obj.send(message_text)
                        print(f"🚀 [THÀNH CÔNG] Đã gửi vào #{target_obj.name}: \"{message_text}\"\n")
                    except Exception as e:
                        print(f"❌ Không thể gửi tin nhắn: {e}\n")
                else:
                    print("❌ Kênh không tồn tại hoặc bot không có quyền gửi tin!\n")

            # 6. LEAVE
            elif command in ["6", "leave"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server nào để out!\n")
                    continue

                confirm = await asyncio.to_thread(input, f"⚠️ Bạn có chắc muốn cho bot RỜI KHỎI [{selected_guild.name}]? (y/n/exit): ")
                confirm = confirm.strip().lower()

                if confirm == "exit":
                    print("🔙 Đã hủy thao tác rời server.\n")
                    continue

                if confirm == 'y':
                    try:
                        g_name = selected_guild.name
                        await selected_guild.leave()
                        print(f"👋 Đã rời khỏi server {g_name} thành công!\n")
                        selected_guild = None
                    except Exception as e:
                        print(f"❌ Lỗi khi rời server: {e}\n")
                else:
                    print("⛔ Đã hủy lệnh leave.\n")

            # 7. INFO
            elif command in ["7", "info"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server! Vui lòng chọn Server trước.\n")
                    continue

                print("\n🔄 Đang lấy thông tin và khởi tạo link mới nhất...")

                try:
                    owner = selected_guild.owner or await selected_guild.fetch_member(selected_guild.owner_id)
                    owner_info = f"{owner.name} (ID: {owner.id})"
                except Exception:
                    owner_info = f"ID: {selected_guild.owner_id}"

                invite_url = "Không có quyền tạo link"
                ...
                text_chs = [f"#{ch.name}" for ch in selected_guild.text_channels]
                voice_chs = [f"🔊 {ch.name}" for ch in selected_guild.voice_channels]

                print("\n==================================================")
                print(f"📊 THÔNG TIN SERVER: {selected_guild.name.upper()}")
                print("==================================================")
                print(f"🆔 ID Server        : {selected_guild.id}")
                print(f"👑 Chủ Server       : {owner_info}")
                print(f"👥 Tổng số thành viên: {selected_guild.member_count} người")
                print(f"🔗 Link Server (Mới): {invite_url}")
                print("--------------------------------------------------")
                print(f"📺 Kênh Chat ({len(text_chs)}): {', '.join(text_chs) if text_chs else 'Không có'}")
                print(f"🔊 Kênh Thoại ({len(voice_chs)}): {', '.join(voice_chs) if voice_chs else 'Không có'}")
                print("==================================================\n")

            # 8. MEMBERS
            elif command in ["8", "members", "users"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server! Vui lòng chọn Server trước.\n")
                    continue

                print(f"\n🔄 Đang lấy danh sách thành viên trong [{selected_guild.name}]...")
                try:
                    members = [m async for m in selected_guild.fetch_members(limit=None)]
                except Exception:
                    members = selected_guild.members

                print(f"\n--- 👥 DANH SÁCH THÀNH VIÊN ({len(members)} người) ---")
                for idx, member in enumerate(members, start=1):
                    bot_tag = " [BOT]" if member.bot else ""
                    nick = f" (Biệt danh: {member.nick})" if member.nick else ""
                    print(f" [{idx}] {member.name}{nick} | ID: {member.id}{bot_tag}")
                print("---------------------------------------------------\n")

            # --------------------------------------------------
            # 9. ADDROLE / ROLE (CẤP ROLE CHO BẠN BẰNG ID)
            # --------------------------------------------------
            elif command in ["9", "addrole", "role"]:
                if not selected_guild:
                    print("❌ Chưa chọn Server! Vui lòng dùng lệnh 3 (select) để chọn server trước.\n")
                    continue

                # Lấy và hiển thị toàn bộ danh sách Vai trò (Role) trong server (loại bỏ role @everyone)
                roles = [r for r in selected_guild.roles if r.name != "@everyone"]
                if not roles:
                    print("❌ Server không có vai trò (Role) nào khả thi!\n")
                    continue

                print(f"\n--- 🎭 DANH SÁCH ROLE TRONG SERVER [{selected_guild.name}] ---")
                for idx, r in enumerate(roles, start=1):
                    print(f" [{idx}] {r.name} | ID Role: {r.id}")
                print("------------------------------------------------------------")

                # Bước 1: Nhập STT hoặc ID của Role
                role_input = await asyncio.to_thread(input, "👉 Nhập STT hoặc ID của Role muốn cấp (gõ 'exit' để hủy): ")
                role_input = role_input.strip()

                if role_input.lower() == "exit":
                    print("🔙 Đã hủy thao tác cấp Role.\n")
                    continue
                ...
                if role_input.isdigit():
                    val = int(role_input)
                    if 1 <= val <= len(roles):
                        target_role = roles[val - 1]
                    else:
                        target_role = selected_guild.get_role(val)

                if not target_role:
                    print("❌ Không tìm thấy Role tương ứng với thông tin bạn nhập!\n")
                    continue

                # Bước 2: Nhập ID của Người Dùng
                user_id_input = await asyncio.to_thread(input, f"👉 Nhập ID Người Dùng cần cấp Role [{target_role.name}] (gõ 'exit' để hủy): ")
                ...
                if not user_id_input.isdigit():
                    print("❌ ID người dùng phải là chuỗi chữ số hợp lệ!\n")
                    continue

                # Bước 3: Tìm người dùng và trao Role
                try:
                    member = selected_guild.get_member(int(user_id_input)) or await selected_guild.fetch_member(int(user_id_input))
                    if member:
                        await member.add_roles(target_role)
                        print(f"🚀 [THÀNH CÔNG] Đã cấp Role '{target_role.name}' cho thành viên {member.name} (ID: {member.id})!\n")
                    else:
                        print("❌ Người dùng này không ở trong Server!\n")
                except discord.Forbidden:
                    print("❌ Lỗi: Bot thiếu quyền 'Manage Roles' hoặc vị trí Role của Bot nằm DƯỚI vị trí Role cần cấp!\n")
                except Exception as e:
                    print(f"❌ Lỗi cấp role: {e}\n")

            # 10. EXIT / UNSELECT
            elif command in ["exit", "unselect", "deselect", "0", "back"]:
                if selected_guild:
                    print(f"🔙 Đã thoát/bỏ chọn Server [{selected_guild.name}]. Quay lại trạng thái ban đầu!\n")
                    selected_guild = None
                else:
                    print("💡 Hiện tại chưa chọn Server nào.\n")

            # HELP
            elif command in ["help", "h"]:
                print("""
--- 📋 BẢNG LỆNH ĐIỀU KHIỂN TERMINAL ---
1. ping                : Kiểm tra độ trễ (latency) của bot.
2. servers             : Xem danh sách Server, ID và Số lượng thành viên.
3. select <STT>        : Chọn Server muốn tương tác (Ví dụ: select 1 hoặc gõ 3).
4. channels            : Xem danh sách các kênh chat của Server đang chọn.
5. send <STT|ID> | <Nội dung> : Gửi thông báo tới kênh chọn.
6. leave               : Cho bot rời khỏi Server đang chọn.
7. info                : Xem chi tiết thông tin Server (Chủ SV, Link, Kênh,...).
8. members / users     : Xem danh sách toàn bộ thành viên trong Server.
9. addrole / role      : Xem danh sách Role + Cấp Role cho người dùng bằng ID.
10. exit / back        : Bỏ chọn Server hiện tại để về trạng thái [Chưa chọn Server].
-----------------------------------------
""")
            else:
                print("❌ Lệnh không hợp lệ! Gõ 'help' hoặc 'h' để xem bảng lệnh.\n")

        except Exception as e:
            print(f"❌ Lỗi Terminal: {e}\n")
